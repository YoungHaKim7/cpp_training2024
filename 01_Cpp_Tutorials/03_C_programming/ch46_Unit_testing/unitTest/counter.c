...
/* destroy a counter */
void counterDestroy(Counter *c) { free(c); }

/* return 1 if counter is 0, 0 otherwise */
int counterIsZero(const Counter *c) { return c->value == 0; }

/* increment a counter, returns 1 if successful, 0 if increment would cause
 * overflow */
...
/* decrement a counter, returns 1 if successful, 0 if decrement would cause
 * underflow */
int counterDecrement(Counter *c) {
  if (c->value == 0) {
    return 0;
  } else {
    c->value--;
